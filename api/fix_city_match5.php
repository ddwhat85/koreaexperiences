<?php
if($_POST['k']!='citymatch5'){http_response_code(403);exit;}
...
if(strpos($h,'citymatch5hook')!==false){echo 'already';exit;}
$idp=strpos($h,'id="expand-cities"');
if($idp===false){echo 'err:no-builder';exit;}
$close=strpos($h,'</script>',$idp);
if($close===false){echo 'err:no-close';exit;}
$map=array(
...
$code=';/*citymatch5hook*/(function(){var M='.$mapJson.';function go(){var cards=document.querySelectorAll(".explore-card.ec-injected");for(var ci=0;ci<cards.length;ci++){var c=cards[ci];var t=(c.textContent||"").replace(/[ \t\r\n]+/g," ").trim();var key=null;for(var k in M){if(t.indexOf(k)===0){if(!key||k.length>key.length){key=k;}}}if(!key){for(var k2 in M){if(t.indexOf(k2)>-1){key=k2;break;}}}if(!key){continue;}var url="https://images.unsplash.com/photo-"+M[key]+"?w=600&h=400&fit=crop&q=80";var els=[c];var inner=c.querySelectorAll("*");for(var j=0;j<inner.length;j++){els.push(inner[j]);}for(var i=0;i<els.length;i++){var st=els[i].getAttribute?els[i].getAttribute("style"):null;if(st&&st.indexOf("url(")>-1){els[i].style.backgroundImage="url("+url+")";}}}}go();setTimeout(go,200);setTimeout(go,800);setInterval(go,500);document.addEventListener("click",function(){setTimeout(go,150);setTimeout(go,500);});})();';
$h=substr($h,0,$close).$code.substr($h,$close);

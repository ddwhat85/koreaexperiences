<?php
if($_POST['k']!='citymatch2'){http_response_code(403);exit;}
...
$s1=strpos($h,'<script>/*citymatch1-marker*/');
if($s1!==false){$e1=strpos($h,'</script>',$s1);if($e1!==false){$h=substr($h,0,$s1).substr($h,$e1+9);}}
$s2=strpos($h,'<script>/*citymatch2-marker*/');
if($s2!==false){$e2=strpos($h,'</script>',$s2);if($e2!==false){$h=substr($h,0,$s2).substr($h,$e2+9);}}
$map=array(
...
$js='<script>/*citymatch2-marker*/(function(){var M='.$mapJson.';function go(){var cards=document.querySelectorAll(".explore-card.ec-injected");for(var ci=0;ci<cards.length;ci++){var c=cards[ci];var t=(c.textContent||"").replace(/[ \t\r\n]+/g," ").trim();var key=null;for(var k in M){if(t.indexOf(k)===0){if(!key||k.length>key.length){key=k;}}}if(!key){for(var k2 in M){if(t.indexOf(k2)>-1){key=k2;break;}}}if(!key){continue;}var url="https://images.unsplash.com/photo-"+M[key]+"?w=600&h=400&fit=crop&q=80";var els=[c];var inner=c.querySelectorAll("*");for(var j=0;j<inner.length;j++){els.push(inner[j]);}for(var i=0;i<els.length;i++){var st=els[i].getAttribute?els[i].getAttribute("style"):null;if(st&&st.indexOf("url(")>-1){els[i].style.backgroundImage="url("+url+")";}}}}go();document.addEventListener("DOMContentLoaded",go);var n=0;var iv=setInterval(function(){go();n++;if(n>40){clearInterval(iv);}},350);document.addEventListener("click",function(){setTimeout(go,150);setTimeout(go,500);});})();</script>';
